...
//    KALLISTO_SE_TRANSCRIPT(KALLISTO_TXIMPORT.out.counts_transcript.concat(KALLISTO_TXIMPORT.out.tpm_transcript).groupTuple(), KALLISTO_TX2GENE.out.tx2gene, ch_samplesheet.map { [ [:], it ] })
    ...
    ch_salmon_fqs_tx2gene = SALMON_FQS_TX2GENE.out.tx2gene.collect()

    //
    // MODULE: Run Salmon pseudoalignment modules
    //
    ...
    SALMON_FQS_SE_GENE(ch_salmon_fqs_counts_gene, ch_salmon_fqs_tpm_gene, ch_salmon_fqs_tx2gene)
    ch_versions = ch_versions.mix(SALMON_FQS_SE_GENE.out.versions)

    SALMON_FQS_SE_GENE_SCALED(ch_salmon_fqs_counts_gene_scaled, ch_salmon_fqs_tpm_gene,ch_salmon_fqs_tx2gene)
    ch_versions = ch_versions.mix(SALMON_FQS_SE_GENE_SCALED.out.versions)

    SALMON_FQS_SE_TRANSCRIPT(ch_salmon_fqs_counts_transcript, ch_salmon_fqs_tpm_transcript, ch_salmon_fqs_tx2gene)
    ch_versions = ch_versions.mix(SALMON_FQS_SE_TRANSCRIPT.out.versions)

    SALMON_FQS_SE_GENE_LENGTH_SCALED(ch_salmon_fqs_counts_gene_length_scaled, ch_salmon_fqs_tpm_gene, ch_salmon_fqs_tx2gene)
